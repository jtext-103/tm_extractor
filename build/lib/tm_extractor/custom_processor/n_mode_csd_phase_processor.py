...
class N_mode_csd_phase(BaseProcessor):
    ...
    def transform(self,  *signal: Signal):
        ...
        for i in range(len(B_LFS_most_signal.data)):
            if B_LFS_most_signal.data[i][0]!=0:
                ...
                #四个频率的最大值
                max_frequency_list = [signal.__getitem__(a).data[i][3] for a in range(0,4)]
                for j in range(len(max_frequency_list)):
                    if max_frequency_list[j]!=0:
                        ...
                        #2324
                        phases_modified =deepcopy(phases)
                    ...
                    # if mode_number_array_i[-1]==0:
                    #     mode_number_array_i[-1] = 1
                    # 调用绘图函数
    ...
    def find_mode_number(self, phases_modified):
        ...
        ########
        if mode_number_csd_phase==0:
            mode_number_csd_phase=1
