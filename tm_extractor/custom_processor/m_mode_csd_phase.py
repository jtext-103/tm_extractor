...
class M_mode_csd_phase(BaseProcessor):
    def __init__(self,theta:List, plt_path: str, plot_phase=False ):
        super().__init__(theta=theta,plt_path=plt_path, plot_phase=plot_phase)
    ...
    def transform(self,  *signal: Signal):
        ...
        for i in range(len(B_LFS_most_signal.data)):
            if B_LFS_most_signal.data[i][0]!=0:
                ...
                #传入一个时间段的阵列
                #2324
                if B_LFS_most_signal.parent.shot_no>=1087715:
    ...
    def find_mode_number(self, phases_modified):
        ...
        for i in range(len(phases_modified)):
            if i == 0:
                ...
                if phases_modified[i - 1] > 0 and phases_modified[i] < 0 and phases_modified[i + 1] >= phases_modified[
                    i] and phases_modified[i + 1] < 0:
                    min_number += 1
                    min_index = np.append(min_index, i)
                if phases_modified[i - 1] > 0 and phases_modified[i] > 0 and phases_modified[i - 1] <= phases_modified[
                    i] and phases_modified[i + 1] < 0:
    ...
    def plot_frequencies_and_phases(self, frequencies, phases, phases_modified, theta, shot_no, i, j, mode_number,
                                    time):
